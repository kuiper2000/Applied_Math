...
_principle of superposition_ then tells us, if today we have a function $y_p$ which equals to $\sum_i Y_i$, $y_p$ is also a particular solution of {eq}`eq27`. Based on this rule, we can decompose $f(x)$ into different pieces which is easier for us to approach. 
